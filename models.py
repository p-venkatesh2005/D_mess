...
class User(UserMixin, db.Model):
    ...
    email = db.Column(db.String(120), nullable=True)  # Removed unique=True to allow duplicates/nulls
    role = db.Column(db.String(20), nullable=False, default='student')  # student, admin, hostler
...
class QRScan(db.Model):
    """Records each student QR scan per meal session."""
    __tablename__ = 'qr_scans'
    id = db.Column(db.Integer, primary_key=True)
    student_id = db.Column(db.Integer, db.ForeignKey('students.id'), nullable=False)
    scan_date = db.Column(db.Date, default=date.today, nullable=False)
    meal_session = db.Column(db.String(20), nullable=False)  # breakfast, lunch, dinner
    scan_time = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)

    student = db.relationship('Student', backref=db.backref('qr_scans', lazy='dynamic'))

    __table_args__ = (
        db.UniqueConstraint('student_id', 'scan_date', 'meal_session',
                            name='unique_scan_per_student_meal_day'),
    )

    def __repr__(self):
        return f'<QRScan student={self.student_id} {self.meal_session} {self.scan_date}>'


class Hostler(db.Model):
    """Profile for hostel residents added by admin."""
    __tablename__ = 'hostlers'
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False, unique=True)
    ...
    user = db.relationship('User', backref=db.backref('hostler', uselist=False))

    def __repr__(self):
        return f'<Hostler {self.user.name}>'


class RoomListing(db.Model):
    """Room-for-rent suggestions posted by admin for mess students."""
    ...
    def __repr__(self):
        return f'<RoomListing {self.title} ₹{self.rent_per_month}>'
